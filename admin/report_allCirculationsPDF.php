...
require_once('includes/load.php');
	...
	if(isset($_GET['catalogID'])) {
		$catalogID = $_GET['catalogID'];
      	$all_circulations_h = find_all_circulations_id($catalogID);
	} elseif(isset($_GET['memberHistory'])) {
		$memberHistory = $_GET['memberHistory'];
      	$all_circulations_h = find_all_circulations_history('member_id',$memberHistory);
		$book_information = find_by_id('members',$memberHistory);
		$page_name = remove_junk(ucwords($book_information['name']));
	} elseif(isset($_GET['catalogHistory'])) {
		$catalogHistory = $_GET['catalogHistory'];
      	$all_circulations_h = find_all_circulations_history('book_id',$catalogHistory);
		$book_information = find_by_id('book_catalog',$catalogHistory);
		$page_name = remove_junk(ucwords($book_information['title']));
	} else {
		$all_circulations_h = find_all_circulations_history_report();
		$page_name = "Troop 61 Library - Historical Bookings";
	}
...
$this->Cell(2.38866666667,0.5,"Member Name",1,'','C');
...
$status  = "Requested";
$status .= "\n";
$status .= remove_junk(read_date($a_circulations_h['date_checked_out']));
$w = 0.3;
...
$this->MultiCell(2.38866666667,0.6,remove_junk(ucwords($a_circulations_h['name'])),1,'C');
...
global $page_name;
    ...
    // Page number
    $this->Cell(0,0.393701,$page_name,0,0,'L');
    $this->SetX(0);
...
$pdf->Write('.5',"Troop 61 Library - Historical Bookings");
